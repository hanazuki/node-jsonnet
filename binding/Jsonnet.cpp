...
namespace nodejsonnet {
  ...
  Napi::Object Jsonnet::init(Napi::Env env) {
    auto const func = DefineClass(env, "Jsonnet", {
        ...
        InstanceMethod("stringOutput", &Jsonnet::setStringOutput),
        ...
        InstanceMethod("evaluateFileStream", &Jsonnet::evaluateFileStream),
        InstanceMethod("evaluateSnippetStream", &Jsonnet::evaluateSnippetStream),
  ...
  Napi::Value Jsonnet::evaluateFileStream(const Napi::CallbackInfo& info) {
    ...
    auto vm = createVm(env);
    auto const worker = new JsonnetWorker(env, vm, std::make_unique<JsonnetWorker::EvaluateFileStreamOp>(std::move(filename)));
    auto const promise = worker->Promise();
    worker->Queue();
    return promise;
  }
  ...
  Napi::Value Jsonnet::evaluateSnippetStream(const Napi::CallbackInfo& info) {
    ...
    auto vm = createVm(env);
    auto const worker = new JsonnetWorker(env, vm, std::make_unique<JsonnetWorker::EvaluateSnippetStreamOp>(std::move(snippet), std::move(filename)));
    auto const promise = worker->Promise();
    worker->Queue();
    return promise;
  }
